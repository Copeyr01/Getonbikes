...
**No layout shift between tabs:** switching "Search by Licence" ↔ "Search by School" used to resize the card (and the whole banner around it, since the banner's height is content-driven) because the two panels have different natural heights. `#licence-form` and `#school-panel` are now both direct children of a `.search-panels` grid wrapper with `grid-area: 1 / 1` — same cell — so the cell's height is always the taller of the two regardless of which is showing. The hidden panel keeps `display:block; visibility:hidden` instead of `display:none` (which the general `.search-panel[hidden]` rule still uses for `#quiz-wrap`, outside this wrapper) — `display:none` would remove it from the grid's height calculation entirely, defeating the point. `align-items: start` on the wrapper stops the shorter panel being stretched to match, which would've pushed the school-search results dropdown down by the height difference.
