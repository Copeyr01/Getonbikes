...
**Two-column layout, new:** the hero is no longer a single centred column — it's a `1fr 1fr` grid, existing content (eyebrow/heading/search/quiz callout/stats, unchanged) on the left in `.hero__content`, a rider photo on the right in `.hero__media`. Direct response to feedback that the site felt visually flat/text-heavy; a real photo gives the hero some actual rich content instead of another block of copy. Collapses to a single stacked column under `960px`. The photo itself hasn't been supplied as a file yet (only seen inline in chat, no accessible path to save it into the repo) — `.hero__media` currently holds a plainly-labelled dashed-border placeholder ("📷 Real rider photo pending upload") rather than a fabricated stock image, consistent with this project's honesty-over-guessing convention elsewhere (No reviews yet, Distance unavailable). Swap to `<img class="hero__image" src="assets/images/hero-rider.jpg">` once the file lands.
