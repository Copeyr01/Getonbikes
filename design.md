...
**Tabs, restyled:** no longer underlined text links — each tab is now its own box (`flex:1`, split evenly), the active one white with its top corners rounded (`var(--radius-xl)`, matching the card) so it reads as "popping up" from the panel below it, the inactive one filled with `--off` (darkened relative to the white card) so there's real visual separation between them. Each tab's own *outer* corner (first tab → top-left, last tab → top-right) is rounded independently of active state, rather than relying on the card clipping it via `overflow:hidden` — that was the first approach, but `overflow:hidden` on the card also clipped the school-search results dropdown, which extends below the card. Functionality (a live search result) wins over a slightly-simpler CSS approach.
