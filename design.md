...
`.hero-banner__search` is a real flex child of `.hero-banner` (not an absolutely-positioned sibling floating over it) — simpler to reason about, and it's what makes the tight text-to-card gap a single `gap` value instead of magic-number positioning. The card keeps the real functionality unchanged, just restyled (small uppercase field labels — "Licence" / "Postcode" — added above each control; fields stack in a column rather than a row): a licence **dropdown** + an optional postcode field + one accent-filled "Find schools" submit, which hands off to `schools.html`.

**"Search by School" tab, removed.** This started as a second hero tab (name search, live results dropdown) alongside "Search by Licence," restyled a few times along the way (see git history on this branch). Reasoned through it directly with the user: the persistent navbar search (`#nav-search-input`, on every page) already does the exact same name/area lookup, so the hero tab wasn't adding a capability, just duplicating one that already exists — and with only 3 schools in the directory total, a name search barely earns a dedicated UI slot yet next to browsing the (nearly-whole) licence-filtered list. Worth revisiting once the directory covers more cities/schools, where "I already know who I want" becomes a real fast-path. Removed: the tab bar, `#school-panel` and its live-dropdown JS (`renderSchoolDropdown`, the associated event listeners), and the `.search-panels` CSS grid-stack trick that existed solely to stop the two tab panels' differing heights from resizing the card on switch — no longer needed with one panel. The navbar's `q`-search JS (a plain Enter-to-submit, no live dropdown) is untouched and still owns this job.

**Fields, de-greyed:** the Licence dropdown and Postcode input used the sitewide `.hero-search` grey (`--off`) fill — inside this card specifically that read as another grey box, so `.hero-banner__search .hero-search` overrides it to white, scoped so the same fields elsewhere (the `schools.html` filter bar, `style-guide.html`) keep their normal fill untouched. The Licence `<select>` also got `appearance: none` plus a custom SVG chevron and an explicit `-webkit-text-fill-color` — its native browser chrome was rendering the selected value in an accent colour on at least one mobile browser regardless of the `color` set on it, which read as an unwanted highlight/box around the text.

**Nav icon buttons, de-boxed:** `.navbar__icon-btn` (the mobile search/menu icons, shared sitewide) dropped its `1px solid var(--border)` box and white fill — sitting right above the new photo hero, the bordered squares read as more chrome than intended. Plain icons now, same click targets and toggle behaviour.
...
**Mobile (≤860px):** `.hero-banner` switches to a column flex layout (text, then card, `16px` gap) instead of a row — the same flex-based structure as desktop, just re-oriented, rather than a separate absolute-positioning scheme. The photo still bleeds edge-to-edge (`width: 100vw` via negative margins). Height is no longer a fixed min-height; it's driven by content, which naturally keeps text and card close together instead of leaving a large empty photo gap between them. (Two earlier iterations of this: one had the card detach into a plain block below the image entirely; the next tried a fixed `620px` min-height with the card absolutely bottom-anchored, which reintroduced the big gap this was meant to solve, plus an unrelated bug — an `overflow-x:hidden` safety-net on the wrapping element was silently clipping the full-bleed trick itself, so the "edge-to-edge" photo wasn't actually reaching the edges despite the CSS looking correct. Removing that rule and moving to a real flex layout fixed both at once.) The scrim switches from the desktop left-to-right gradient to a top-to-bottom one, since the heading now sits above a full-width card instead of a photo's clearer half.

**Corners:** `.hero-banner` is `border-radius: 0 0 20px 20px` at every width — square top, rounded bottom. On desktop this means the top corners no longer round off; on mobile, since the photo already sits flush under the navbar (no gap, see above), a square top read as correct there too, but the bottom needed rounding to soften the transition into the white page below instead of a hard edge-to-edge cut. One rule serves both breakpoints since neither overrides it.
