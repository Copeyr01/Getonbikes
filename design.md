...
### Hero
**Full-bleed banner, new** (superseding the earlier two-column `hero__grid` split from the same design pass): a single large `20px`-rounded photo panel (`assets/images/hero-rider.webp`, `object-fit: cover`) spans the full container width, with a dark left-to-right scrim gradient for text legibility. White heading + subtitle sit over the image's left side; a floating white search card (`.hero-banner__search`) sits over the right side, vertically centred, `16px`/`40px` box-shadow lifting it off the photo. Below the banner, a plain stats line (`Now live in Edinburgh — 3 Edinburgh schools listed, 4 licence types covered, 0 reviews so far`) replaces the old on-image eyebrow. Modelled directly on a reference screenshot the user supplied (a travel-booking hero: big photo, contained floating search widget with tabs) — not the source `Design__Carwowstyle.md` doc, which only specified the two-column version this replaces.

The floating card itself keeps the real functionality unchanged, just restyled to fit a narrower, vertically-stacked context (small uppercase field labels — "Licence" / "Postcode" / "School name" — added above each control, a first for this project; fields stack in a column rather than a row):
...
**Mobile (≤860px):** the photo bleeds edge-to-edge (`width: 100vw` via negative margins, corners square instead of rounded), grows taller (`620px` min-height) to give the stacked content room, and the search card stays overlaid on it — bottom-anchored, near-full-width within the container's normal side padding — rather than detaching into a plain block below. (An earlier version of this pass had it detach to match a reference mobile mockup exactly; live-testing that felt wrong, so the card went back to floating on the image at every width, matching how the desktop version already worked.) The scrim switches from the desktop left-to-right gradient to a top-to-bottom one, since the heading now sits above a full-width card instead of a photo's clearer half.
