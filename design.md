...
**Two-column layout, new:** the hero is no longer a single centred column — it's a `1fr 1fr` grid, existing content (eyebrow/heading/search/quiz callout/stats, unchanged) on the left in `.hero__content`, a real rider photo (`assets/images/hero-rider.webp`) on the right in `.hero__media`, `object-fit: cover` at `10–12px` rounded corners matching the card radius elsewhere. Direct response to feedback that the site felt visually flat/text-heavy — a real photo gives the hero some actual rich content instead of another block of copy. Collapses to a single stacked column under `960px`.
